...
cl_header2 "$Msf - BrazilFW"
 ...
 add_control "$SCRIPT?ACTION=UPDATEL7" "Atualizar Protocolos L7"
 end_add_control
 end_table
 echo "<br>"

if [ "$FORM_ACTION" = "UPDATEL7" ]; then
# Atualiza os protocolos do Layer 7
# Autor: Bender
 cd /etc/l7-protocols/
 for J in protocols extra file_types malware; do
    wget http://l7-filter.sourceforge.net/layer7-protocols/$J >/dev/null 2>&1
    PROTS=`grep ".pat" $J |cut -d'"' -f8`
    for I in $PROTS; do
     echo "<div>Baixando: $J - $I</div>"
     wget http://l7-filter.sourceforge.net/layer7-protocols/$J/$I >/dev/null 2>&1 &
    done
    rm $J
 done
 sleep 15
 echo "<center><div>Protocolos Layer 7 Atualizados!</center></div>"
 echo "<center><div id=back><a href=$SCRIPT class=links>$Fbl</a></div></center>"
 cl_footer2
 exit 0
fi
...
cat << CLEOF
 <form method="POST" action="$SCRIPT"><table class=maintable border=0 width=100%>
 <tr>
  <th colspan=4>$Pna</th>
 </tr>
 <tr>
  <td width="50%" colspan=2 class=row1><input type=radio value=ALLOW_ALL name=DEFAULT_USERS_FILTER `[ "$DEFAULT_USERS_FILTER" != "BLOCK_ALL" ] && echo -n checked` ><b>$Pnd</b></td>
 ...
 <tr>
  <td width="25%" class=row1 align=left valign=top><b>$Pnj</b><br>$Pnm  $Pnl<br>192.168.0.2<br>10.0.0.2</td>
 ...
 <tr>
  <td colspan=2 class=row2 align=left valign=top><b>$Pts</b><br>$Ptt<br>192.168.0.10 01:0d:03:0e:00:0f<br>192.168.0.11 01:0d:03:0e:dd:ff<br><br>$Ptu</td>
  <td colspan=2 class=row2><textarea rows=7 cols=44 name=IP_MAC_LIST wrap=off>`cat $IP_MAC_LIST`</textarea></td>
 </tr>
 <tr>
  <th colspan=4 class=row3>$Pnb</th>
 </tr>
 <tr>
  <td colspan=2 class=row1><input type=radio value=ALLOW_ALL name=DEFAULT_SERVICES_FILTER `[ "$DEFAULT_SERVICES_FILTER" != "BLOCK_ALL" ] && echo -n checked` ><b>$Pnh</b></td>
 ...
 <tr>
  <td class=row1 align=left valign=top><b>$Pnj</b><br>$Pnq  $Pnl<br>80 tcp<br>53 udp<br>20:21 tcp</td>
...
cat << CLEOF
 ...
 <tr>
  <td colspan=2 class=row3 align=left valign=top>$Pnt<br>$Pnu<br>fasttrack<br>edonkey<br>directconnect<br><br>
