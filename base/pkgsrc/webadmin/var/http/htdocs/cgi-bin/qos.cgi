...
# Process Form Submission
	...
	MAINCONFIGCHANGED=
	if [ "$FORM_QOS_TYPE" != "$QOS_TYPE" ]; then
		QOS_TYPE=$FORM_QOS_TYPE
		MAINCONFIGCHANGED=yes
	fi
	if [ "$FORM_QOS_DOWNSTREAM" != "$QOS_DOWNSTREAM" ]; then
		QOS_DOWNSTREAM=$FORM_QOS_DOWNSTREAM
	fi
	if [ "$FORM_QOS_UPSTREAM" != "$QOS_UPSTREAM" ]; then
		QOS_UPSTREAM=$FORM_QOS_UPSTREAM
	fi
